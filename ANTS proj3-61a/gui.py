...
class GUI:
    ...
    def makeHooks(self):
        ...
        # Enable the Remover tool in the web GUI
        ants.AntRemover.implemented = True
    ...
    def get_insect_img_file(self, name):
        return INSECT_FILES.get(name, ASSETS_DIR + INSECT_DIR + "bee.gif")
    ...
    def _update_control_panel(self, gamestate):
        ...
        for name, place in gamestate.places.items():
            ...
            if place.ant is not None:
                if place.ant not in self.insectToId:
                    self.insectToId[place.ant] = self.currentInsectId
...
class HttpHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def parse_post_data(self):
        """Parse application/x-www-form-urlencoded POST bodies (Python 3.13+ safe)."""
    ...
    def do_POST(self):
        ...
        # deployAnt expects nested JSON in form fields in older clients;
        # app.js sends pname/ant as form fields, and may send JSON string in "data"
        if 'pname' in data and 'ant' in data:
            pass
        elif 'data' in data:
            try:
